...
value: function (t, e, i) {
  ...
  const labelText = t.label.text;
  let y1 = this._getY1Y2("y1", t);
  let y2 = t.y2 ? this._getY1Y2("y2", t) : null;
  let hasRect = typeof t.isRectangle !== "undefined" && t.isRectangle;

  // === Calculate X Position ===
  let calculatedX = 0;
  if (typeof t.x === "string") {
    let idx = s.globals.labels.indexOf(t.x);
    if (s.config.xaxis.convertedCatToNumeric)
      idx = s.globals.categoryLabels.indexOf(t.x);
    calculatedX = this.annoCtx.helpers.getStringX(t.x);
  } else if (typeof t.x === "number") {
    calculatedX =
      (t.x - s.globals.minX) / (s.globals.xRange / s.globals.gridWidth);
  }

  // === Draw Line or Rectangle ===
  if (!t.y2) {
    // --- Horizontal line ---
    if (y1 < s.globals.gridHeight && Math.sign(y1) !== -1) {
      const line = this.annoCtx.graphics.drawLine(
        calculatedX + (t.offsetX || 0),
        y1 + (t.offsetY || 0),
        s.globals.gridWidth + (t.offsetX || 0),
        y1 + (t.offsetY || 0),
        t.borderColor,
        r,
        t.borderWidth || 2
      );
      e.appendChild(line.node);
      if (t.id) line.node.classList.add(t.id);
    }
  } else {
    // --- Rectangle region ---
    ...
    if (hasRect) {
      // Draw rectangle starting from calculatedX
      calculatedX = Math.max(calculatedX, 0);
      const rect = this.annoCtx.graphics.drawRect(
        calculatedX + (t.offsetX || 0),
        topY + (t.offsetY || 0),
        s.globals.dom.elGraphical.node.getBBox().width + (t.offsetX || 0),
        bottomY - topY,
        0,
        t.fillColor,
        t.opacity,
      ...
      rect.attr(
        "clip-path",
        "url(#gridRectMask" + s.globals.cuid + ")"
      );
      e.appendChild(rect.node);
      if (t.id) rect.node.classList.add(t.id);
    } else if (t.type === "diagonal") {
  // === Diagonal annotation ===
  const s = this.w;
  const r = t.strokeDashArray || 0;
  const strokeWidth = t.borderWidth || 2;
  const color = t.borderColor || "#FF4560";

  // Convert data X values to pixel coordinates
  const x1 = (t.x1 - s.globals.minX) / (s.globals.xRange / s.globals.gridWidth);
  const x2 = (t.x2 - s.globals.minX) / (s.globals.xRange / s.globals.gridWidth);

  // Convert Y values using existing helper
  const y1 = this._getY1Y2("y1", { y: t.y1, yAxisIndex: t.yAxisIndex || 0 });
  const y2 = this._getY1Y2("y2", { y2: t.y2, yAxisIndex: t.yAxisIndex || 0 });
  ...
  line.node.classList.add("apexcharts-diagonal-annotation");
  if (t.id) line.node.classList.add(t.id);
  e.appendChild(line.node);

  // Optional label
  if (t.label && t.label.text) {
    const midX = (x1 + x2) / 2 + (t.label.offsetX || 0);
    const midY = (y1 + y2) / 2 + (t.label.offsetY || 0);
    const textEl = this.annoCtx.graphics.drawText({
      x: midX,
      y: midY,
      text: t.label.text,
      textAnchor: "middle",
      foreColor: t.label.style?.color || "#fff",
      fontSize: t.label.style?.fontSize || "12px",
      cssClass: "apexcharts-diagonal-annotation-label",
    });
    e.appendChild(textEl.node);
  }
}
else {
      // Default rectangular span (no custom flag)
      const rect = this.annoCtx.graphics.drawRect(
        0 +
          -(
            (this.w.globals.dom.elGraphical.node.getBBox().width +
              (931 - this.w.globals.dom.elGraphical.node.getBBox().width) -
  ...
  let labelX, labelY;
  if (hasRect) {
    // Center label vertically in rectangle
    labelX =
      "right" === t.label.position
        ? s.globals.gridWidth + (t.label.offsetX || 0)
        : calculatedX + (t.label.offsetX || 0);
    labelY = (y1 + y2) / 2 + (t.label.offsetY || 0);
  } else {
    // Above the line
    labelX =
      typeof t.x === "number"
        ...
        ? s.globals.gridWidth + (t.label.offsetX || 0)
        : (t.label.offsetX || 0);
    labelY = y1 + (t.label.offsetY || 0) - 8; // slightly above
  }
				...
				value: function(t, e, i, a) {
					...
					if (Array.isArray(o.x))
						for (var d = 0; d < o.x.length; d++) {
							...
							if ("line" !== r.config.chart.type && "area" !== r.config.chart.type || r.globals.comboCharts || r.config.tooltip.intersect || (f += " no-pointer-events"), (Array.isArray(r.config.markers.size) ? r.globals.markers.size[e] > 0 : r.config.markers.size > 0) || s || c) {
								...
								if (r.config.series[n].data[g] && (r.config.series[n].data[g].fillColor && (x.pointFillColor = r.config.series[n].data[g].fillColor), r.config.series[n].data[g].strokeColor && (x.pointStrokeColor = r.config.series[n].data[g].strokeColor)), void 0 !== a && (x.pSize = a), (o.x[d] < -r.globals.markers.largestSize || o.x[d] > r.globals.gridWidth + r.globals.markers.largestSize || o.y[d] < -r.globals.markers.largestSize || o.y[d] > r.globals.gridHeight + r.globals.markers.largestSize) && (x.pSize = 0), !p)(r.globals.markers.size[e] > 0 || s || c) && !l && (l = h.group({
									class: s || c ? "" : "apexcharts-series-markers"
								})).attr("clip-path", "url(#gridRectMarkerMask".concat(r.globals.cuid, ")")), (u = h.drawMarker(o.x[d], o.y[d], x)).attr("rel", g), u.attr("j", g), u.attr("index", e), u.node.setAttribute("default-marker-size", x.pSize), new Li(this.ctx).setSelectionFilter(u, e, g), this.addEvents(u), l && l.add(u)
							} else void 0 === r.globals.pointsArray[e] && (r.globals.pointsArray[e] = []), r.globals.pointsArray[e].push([o.x[d], o.y[d]])
						}
					return l
				}
			}, {
			...
			return s(t, [{
				...
				value: function() {
					...
					return new Promise((function(e) {
						...
						1 !== s && t.scaleSvgNode(n, s), t.convertImagesToBase64(n).then((function() {
				...
				value: function(t) {
					...
					return new Promise((function(i) {
						...
						var n = "transparent" !== a.config.chart.background && a.config.chart.background ? a.config.chart.background : "#fff",
							o = r.getContext("2d");
						o.fillStyle = n, o.fillRect(0, 0, r.width * s, r.height * s), e.getSvgString().then((function(t) {
					...
					if (s.config.grid.xaxis.lines.show || s.config.xaxis.axisTicks.show) {
						...
						s.globals.timescaleLabels.length ? function(t) {
							...
							y2: o
								...
								t.xC=9; // mns added to limit the number of vertical line 
					...
					for (var p = t; p < a; p++) {
						...
						for (var m = x; m <= b && m < r.series[p].length; m++) {
							...
							if (null !== y && v.isNumber(y)) {
								...
								})), i = n, v.isFloat(y) && (y = v.noExponents(y), r.yValueDecimal = Math.max(r.yValueDecimal, y.toString().split(".")[1].length)), o > h[p][m] && h[p][m] < 0 && (o = h[p][m])
			...
			return s(a, [{
				...
				value: function(t) {
					...
					if (a.config.yaxis.forEach((function(t, e) {
							...
							d.push(a.globals.yAxisScale[e].niceMax - r.yRatio[i] * s.startY), u.push(a.globals.yAxisScale[e].niceMax - r.yRatio[i] * s.endY)
						...
						if (a.globals.zoomEnabled) {
							...
							a.config.chart.group || (x.yaxis = g), s.ctx.updateHelpers._updateOptions(x, !1, s.w.config.chart.animations.dynamicAnimation.enabled), "function" == typeof a.config.chart.events.zoomed && n.zoomCallback(p, g)
		...
		ya = function() {
			...
			return s(t, [{
				...
				value: function(t) {
					...
					b && (b.innerHTML = void 0 !== c ? c : ""), f[0] && f[0].classList.contains("apexcharts-tooltip-marker") && (l.config.tooltip.marker.fillColors && Array.isArray(l.config.tooltip.marker.fillColors) && (o = l.config.tooltip.marker.fillColors[e]), f[0].style.backgroundColor = o), l.config.tooltip.marker.show || (f[0].style.display = "none");
				...
				value: function(t) {
					...
					"string" == typeof l ? n.innerHTML = l : (l instanceof Element || "string" == typeof l.nodeName) && (n.innerHTML = "", n.appendChild(l))
				}
			}]), t
		}(),
		wa = function() {
			...
			return s(t, [{
				...
				value: function(t) {
					for (var e = this, i = this.w, a = [], s = this.getElTooltip(), r = function(r) {
							...
							o.classList.add("apexcharts-tooltip-marker"), o.style.backgroundColor = i.globals.colors[r], n.appendChild(o);
							...
							})), n.appendChild(l), s.appendChild(n), a.push(n)
						}, n = 0; n < t; n++) r(n);
					return a
				}
			}, {
			...
			return s(t, [{
				...
				value: function(t) {
					...
					return L = this.barCtx.isHorizontal ? this.calculateBarsDataLabelsPosition(E) : this.calculateColumnsDataLabelsPosition(E), m.attr({
						...
						x: L.dataLabelsX,
						y: L.dataLabelsY,
						val: this.barCtx.isRangeBar ? [r, n] : "100%" === v.config.chart.stackType ? d[h][l] : v.globals.series[h][l],
						i: h,
						j: l,
						barWidth: p,
						barHeight: g,
				...
				value: function(t) {
					...
					if (this.barCtx.isVerticalGroupedRangeBar ? u += h / 2 : (u = e.globals.isXNumeric ? l - h / 2 + m : l - A + h / 2 + m, k > 0 && e.config.plotOptions.bar.hideZeroBarsWhenGrouped && (u -= h * k)), w) {
					...
					switch (this.barCtx.isReversed && (S = o + (C ? c : -c)), f.position) {
						case "center":
							g = w ? C ? S - c / 2 + v : S + c / 2 - v : C ? S - c / 2 + d.height / 2 + v : S + c / 2 + d.height / 2 - v;
							break;
						case "bottom":
					...
					if (this.barCtx.lastActiveBarSerieIndex === s && x.enabled) {
						var L = new Mi(this.barCtx.ctx).getTextRects(this.getStackedTotalDataLabel({
							realIndex: s,
							j: a
						}), p.fontSize);
						r = C ? S - L.height / 2 - v - x.offsetY + 18 : S + L.height + v + x.offsetY - 18;
						var M = A;
						n = y + (e.globals.isXNumeric ? -h * e.globals.barGroups.length / 2 : e.globals.barGroups.length * h / 2 - (e.globals.barGroups.length - 1) * h - M) + x.offsetX
					}
					return e.config.chart.stacked || (g < 0 ? g = 0 + b : g + d.height / 3 > e.globals.gridHeight && (g = e.globals.gridHeight - b)), {
						bcx: l,
						bcy: o,
						dataLabelsX: u,
						dataLabelsY: g,
				...
				value: function(t) {
					...
					l = Math.abs(l);
					var m, v, y = n - (this.barCtx.isRangeBar ? 0 : b) + o / 2 + h.height / 2 + x - 3,
						w = "start",
					...
					if (this.barCtx.lastActiveBarSerieIndex === r && p.enabled) {
						var C = new Mi(this.barCtx.ctx).getTextRects(this.getStackedTotalDataLabel({
							realIndex: r,
							j: s
						}), u.fontSize);
						k ? (m = A - d - f - p.offsetX, w = "end") : m = A + f + p.offsetX + (this.barCtx.isReversed ? -(l + d) : d), v = y - h.height / 2 + C.height / 2 + p.offsetY + d
					}
			...
			return s(t, [{
				...
				value: function() {
					...
					if (l.config.plotOptions.bar.rangeBarGroupRows && (c = 1), this.barCtx.isHorizontal) s = (i = l.globals.gridHeight / h) / c, l.globals.isXNumeric && (s = (i = l.globals.gridHeight / this.barCtx.totalItems) / this.barCtx.seriesLen), s = s * parseInt(this.barCtx.barOptions.barHeight, 10) / 100, -1 === String(this.barCtx.barOptions.barHeight).indexOf("%") && (s = parseInt(this.barCtx.barOptions.barHeight, 10)), o = this.barCtx.baseLineInvertedY + l.globals.padHorizontal + (this.barCtx.isReversed ? l.globals.gridWidth : 0) - (this.barCtx.isReversed ? 2 * this.barCtx.baseLineInvertedY : 0), this.barCtx.isFunnel && (o = l.globals.gridWidth / 2), e = (i - s * this.barCtx.seriesLen) / 2;
						...
						} - 1 === String(this.barCtx.barOptions.columnWidth).indexOf("%") && (r = parseInt(this.barCtx.barOptions.columnWidth, 10)), n = l.globals.gridHeight - this.barCtx.baseLineY[this.barCtx.translationsIndex] - (this.barCtx.isReversed ? l.globals.gridHeight : 0) + (this.barCtx.isReversed ? 2 * this.barCtx.baseLineY[this.barCtx.translationsIndex] : 0), t = l.globals.padHorizontal + v.noExponents(a - r * this.barCtx.seriesLen) / 2
					}
		...
		Pa = function() {
			...
			return s(t, [{
				...
				value: function(t) {
					...
					var I = s / L.config.chart.animations.animateGradually.delay * (L.config.chart.animations.speed / L.globals.dataPoints) / 2.4,
						T = M.renderPaths({
							i: r,
							j: s,
							realIndex: e,
				...
				value: function(t) {
					...
					if (c.globals.isXNumeric) e = (o = (c.globals.seriesX[d][u] - c.globals.minX) / this.invertedXRatio - a) + a * this.visibleI;
						...
						c.globals.seriesPercent.forEach((function(t, e) {
							t[u] && g++, e < d && 0 === t[u] && p++
						})), g > 0 && (a = this.seriesLen * a / g), e = o + a * this.visibleI, e -= a * p
		...
		Ta = function(t) {
			...
			return s(a, [{
				...
				value: function(t) {
					...
					var L = l.move(S, r),
						M = l.move(S + s / 2, k);
					return o.globals.previousPaths.length > 0 && (M = this.getPreviousPath(g, c, !0)), L = this.isBoxPlot ? [l.move(S, k) + l.line(S + s / 2, k) + l.line(S + s / 2, v) + l.line(S + s / 4, v) + l.line(S + s - s / 4, v) + l.line(S + s / 2, v) + l.line(S + s / 2, k) + l.line(S + s, k) + l.line(S + s, C) + l.line(S, C) + l.line(S, k + n / 2), l.move(S, C) + l.line(S + s, C) + l.line(S + s, A) + l.line(S + s / 2, A) + l.line(S + s / 2, y) + l.line(S + s - s / 4, y) + l.line(S + s / 4, y) + l.line(S + s / 2, y) + l.line(S + s / 2, A) + l.line(S, A) + l.line(S, C) + "z"] : [l.move(S, A) + l.line(S + s / 2, A) + l.line(S + s / 2, v) + l.line(S + s / 2, A) + l.line(S + s, A) + l.line(S + s, k) + l.line(S + s / 2, k) + l.line(S + s / 2, y) + l.line(S + s / 2, k) + l.line(S, k) + l.line(S, A - n / 2)], M += l.move(S, k), o.globals.isXNumeric || (i += a), {
						pathTo: L,
						pathFrom: M,
						x: i,
						y: A,
			...
			return s(t, [{
				...
				value: function(t) {
					...
					if (0 === n) {
						...
						l.globals.isXNumeric && (d = (l.globals.seriesX[i][0] - l.globals.minX) / this.lineCtx.xRatio + l.config.markers.offsetX), h.push(d), c.push(v.isNumber(e[r][0]) ? o + l.config.markers.offsetY : null), h.push(a + l.config.markers.offsetX), c.push(v.isNumber(e[r][n + 1]) ? s + l.config.markers.offsetY : null)
					} else h.push(a + l.config.markers.offsetX), c.push(v.isNumber(e[r][n + 1]) ? s + l.config.markers.offsetY : null);
					return {
						x: h,
						y: c
					}
					...
					return t.forEach((function(t) {
						...
						n.forEach((function(r, n) {
							var h = r[0],
								...
								d = r[2],
								u = r[3],
								g = a.drawRect(h, c, d - h, u - c, i.config.plotOptions.treemap.borderRadius, "#fff", 1, e.strokeWidth, i.config.plotOptions.treemap.useFillColorAsStroke ? f : i.globals.stroke.colors[o]);
							g.attr({
								cx: h,
								cy: c,
								index: o,
							...
							if (i.globals.dataChanged) {
								var y = 1;
								e.dynamicAnim.enabled && i.globals.shouldAnimate && (y = e.dynamicAnim.speed, i.globals.previousPaths[o] && i.globals.previousPaths[o][n] && i.globals.previousPaths[o][n].rect && (b = i.globals.previousPaths[o][n].rect), e.animateTreemap(g, b, m, y))
							}
							...
							"truncate" === i.config.plotOptions.treemap.dataLabels.format && (w = parseInt(i.config.dataLabels.style.fontSize, 10), k = e.truncateLabels(k, w, h, c, d, u));
							var A = null;
							i.globals.series[o][n] && (A = e.helpers.calculateDataLabels({
								text: k,
								x: (h + d) / 2,
								...
								fontSize: w,
								series: t
							})), i.config.dataLabels.enabled && A && e.rotateToFitLabel(A, w, k, h, c, d, u), l.add(g), null !== A && l.add(A)
						})), l.add(c), r.add(l)
					})), r
				}
			}, {
		...
		qa = function() {
			...
			return s(t, [{
				...
				value: function(t, e) {
					...
					})), h > 0 && (l && console.warn("Chart or series type ".concat(l, " cannot appear with other chart or series types.")), n.column.series.length > 0 && s.plotOptions.bar.horizontal && (h -= n.column.series.length, n.column = {
						series: [],
						i: []
